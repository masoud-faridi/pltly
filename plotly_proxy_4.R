...
ui <- fluidPage(  
  shinyWidgets::pickerInput('grp_levels',  
                            ...
                            selected = sort(unique(df_1$grp), decreasing = TRUE),  
...
server <- function(input, output, session) {  
  ...
  observeEvent(c(input$grp_levels, input$grp2_levels, input$grp3_levels), {  
    # When groups are selected, update the plot accordingly  
    plotlyProxy("plotly_sunburst", session) %>%  
      plotlyProxyInvoke("restyle", list(visible = "TRUE"), list(1, 2, 3))  # Update markers for all three sets  
  })  
}  
